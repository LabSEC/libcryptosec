...
CustomECDSAKeyPair::~CustomECDSAKeyPair() {
	// TODO Auto-generated destructor stub
}

CustomECDSAKeyPair::CustomECDSAKeyPair(ByteArray& derEncoded) throw (AsymmetricKeyException) {
	this->key = NULL;
	this->engine = NULL;
...
CustomECDSAKeyPair::CustomECDSAKeyPair(std::string& encoded) throw (AsymmetricKeyException) {
	this->key = NULL;
	this->engine = NULL;
...
CustomECDSAKeyPair::CustomECDSAKeyPair(const EllipticCurve & curve) throw (AsymmetricKeyException) {
	this->key = NULL;
	this->engine = NULL;
...
void CustomECDSAKeyPair::generateKey(EC_GROUP * group) throw (AsymmetricKeyException) {

	EC_KEY* eckey = EC_KEY_new();

	if (eckey == NULL){
		throw AsymmetricKeyException(AsymmetricKeyException::INTERNAL_ERROR,
				"Failed initiate EC_KEY", "CustomECDSAKeyPair::generateKey");
	}
	...
	if (EC_KEY_set_group(eckey, group) == 0){
		...
		EC_GROUP_free(group);
		throw AsymmetricKeyException(AsymmetricKeyException::INTERNAL_ERROR,
				"Failed to set group", "CustomECDSAKeyPair::generateKey");
	}
	...
	if (!EC_KEY_generate_key(eckey)) {
		...
		EC_GROUP_free(group);
		throw AsymmetricKeyException(AsymmetricKeyException::INTERNAL_ERROR,
				"Failed to generate keys", "CustomECDSAKeyPair::generateKey");
	}
	...
	if (!this->key) {
		...
		EC_GROUP_free(group);
		throw AsymmetricKeyException(AsymmetricKeyException::INTERNAL_ERROR,
				"Failed to assert EC_KEY into EVP_KEY", "CustomECDSAKeyPair::generateKey");
	}

}

EC_GROUP * CustomECDSAKeyPair::createGroup(const EllipticCurve& curve) {
	BN_CTX *ctx;
	EC_GROUP *group;
	EC_POINT *generator;
	...
	if (ctx == NULL){
		throw AsymmetricKeyException(AsymmetricKeyException::INTERNAL_ERROR,
				"Failed to create BN_CTX", "CustomECDSAKeyPair::createGroup");
	}
	...
	if (group == NULL) {
		BN_CTX_free(ctx);
		throw AsymmetricKeyException(AsymmetricKeyException::INTERNAL_ERROR,
				"Failed to create group", "CustomECDSAKeyPair::createGroup");
	}
	...
	if (generator == NULL) {
		...
		EC_GROUP_free(group);
		throw AsymmetricKeyException(AsymmetricKeyException::INTERNAL_ERROR,
				"Failed to create generator", "CustomECDSAKeyPair::createGroup");
	}
	...
	if (1 != EC_POINT_set_affine_coordinates_GFp(group, generator, curve.BN_x(), curve.BN_y(), ctx)) {
		...
		throw AsymmetricKeyException(AsymmetricKeyException::INTERNAL_ERROR,
				...
				"CustomECDSAKeyPair::createGroup");
	}
	...
	if (1 != EC_GROUP_set_generator(group, generator, curve.BN_order(),	curve.BN_cofactor())) {
		...
		throw AsymmetricKeyException(AsymmetricKeyException::INTERNAL_ERROR,
				"Failed to set generator and order", "CustomECDSAKeyPair::createGroup");
	}
...
EC_GROUP * CustomECDSAKeyPair::createGroup(ByteArray &derEncoded) {
	this->key = NULL;
	this->engine = NULL;
	EC_GROUP *group = NULL;
	BIO * in;
	...
	if ((in == NULL)) {
		throw AsymmetricKeyException(AsymmetricKeyException::INTERNAL_ERROR,
						"Failed to initiate BIO", "CustomECDSAKeyPair::createGroup");
	}
