...
ECDSAKeyPair::ECDSAKeyPair(const Curve & curve) throw (AsymmetricKeyException) {
	this->key = NULL;
	this->engine = NULL;
...
EC_GROUP * ECDSAKeyPair::createGroup(const Curve& curve) {
	BN_CTX *ctx;
	EC_GROUP *group;
	EC_POINT *generator;
...
EC_GROUP * ECDSAKeyPair::createGroup(ByteArray &derEncoded) {
	...
	if ((in == NULL)) {
		throw AsymmetricKeyException(AsymmetricKeyException::INTERNAL_ERROR,
						"Failed to initiate BIO", "ECDSAKeyPair::ECDSAKeyPair");
	}
	...
	if(group == NULL){
		BIO_free(in);
		throw AsymmetricKeyException(AsymmetricKeyException::INTERNAL_ERROR,
				"Failed to create group", "ECDSAKeyPair::ECDSAKeyPair");
	}
